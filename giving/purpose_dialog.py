...
class PurposeRepository:
    ...
    def destinations(self, organization_id):
        ...
        accounts = self.all(
            "SELECT ID,CONCAT(Code,' - ',Name) FROM tblAccountingAccount "
    ...
    def purpose(self, purpose_id):
        rows = self.all(
            ...
            "EffectiveFrom,EffectiveThrough,IsActive,OrganizationID,FundID,RevenueAccountID,"
    ...
    def save(self, purpose_id, values):
        (name, description, approval_date, authority, effective_from, effective_through,
         active, organization_id, fund_id, account_id, confirmed, treatment, note) = values
        if not name:
        ...
        try:
            if purpose_id is None:
                cursor.execute(
                    ...
                    "EffectiveThrough,IsActive,OrganizationID,FundID,RevenueAccountID,"
                ...
                cursor.execute(
                    ...
                    "OrganizationID=?,FundID=?,RevenueAccountID=?,ControlAndDiscretionConfirmed=?,"
...
class PurposeDialog(wx.Dialog):
    ...
    def __init__(self, parent, connection):
        ...
        self.organizations = self.repository.organizations(); self.funds = []; self.accounts = []
        ...
        self.fund=wx.Choice(panel); self.account=wx.Choice(panel)
        ...
        for label,control in (("Purpose name",self.name),("Description",self.description),
            ...
            ("Revenue account",self.account),("",self.confirmed),("Statement treatment",self.treatment),
            ("Note",self.note)):
    ...
    def on_organization(self,_event=None):
        ...
        self.funds,self.accounts=self.repository.destinations(self.organizations[selected][0]) if selected>=0 else ([],[])
    ...
    def values(self):
        return (self.name.GetValue().strip(),self.description.GetValue().strip() or None,
            ...
            self._choose_id(self.accounts,self.account),int(self.confirmed.GetValue()),
    ...
    def on_select(self,_event=None):
        ...
        self.active.SetValue(bool(row[7])); self.confirmed.SetValue(bool(row[11])); self.note.SetValue(row[13])
        ...
        self.treatment.SetSelection([item[1] for item in self.TREATMENTS].index(row[12]))
