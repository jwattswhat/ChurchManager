"""Approved data guards and normalization hooks for ChurchManager migrations."""
...
def before_migration(cursor, record):
    """Run the approved precondition for one migration, when required."""
    if record.version == OBSOLETE_STRUCTURE_CLEANUP:
        verify_obsolete_structure_conversion(cursor)


def after_migration(cursor, record):
    """Run the approved deterministic normalization after one migration."""
