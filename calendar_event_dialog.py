...
class EventEditorDialog(wx.Dialog):
    """Edit one safe event and its controlled natural-language schedule."""
    ...
    def __init__(self, parent, event=None):
        ...
        note = wx.StaticText(panel, label="Enter public-safe event information. Use a natural schedule such as Every Tuesday or Once on October 1, 2026.")
        ...
        self.schedule = wx.TextCtrl(panel, value=str(self.event.get("schedule_text") or ""))
        ...
        if not self.schedule.GetValue():
            self.schedule.SetValue(f"Once on {starts.strftime('%B')} {starts.day}, {starts.year}")
        ...
        for label, control in (("Title", self.title), ("First date", self.date), ("Repeats", self.schedule), ("Start time", self.start),
    ...
    def values(self):
        ...
        return {**self.event, "title": self.title.GetValue(), "starts_at": starts, "ends_at": ends,
                "schedule_text": self.schedule.GetValue(),
...
class CalendarEventsDialog(wx.Dialog):
    ...
    def __init__(self, parent, connection, session, authorization):
        ...
        guidance = wx.StaticText(panel, label="A simple event list using natural schedules. Reminders and invitations remain in your external calendar.")
        ...
        for index, (label, width) in enumerate((("First date",110),("Repeats",180),("Event",230),("Location",150),("Status",90),("Calendar",80))): self.list.InsertColumn(index,label,width=width)
    ...
    def refresh(self):
        ...
        for row in self.rows:
            ...
            for column, value in enumerate((row["schedule_text"], row["title"], row["location"] or "", row["status"].title(), "Yes" if row["calendar_eligible"] else "No"), 1): self.list.SetItem(index,column,str(value))
