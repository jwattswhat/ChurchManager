...
class BudgetDialog(wx.Dialog):
    def __init__(self,parent,service):
        ...
        new=wx.Button(self,label="New Budget");add=wx.Button(self,label="Add Line");edit=wx.Button(self,label="Edit Line");close=wx.Button(self,wx.ID_CLOSE,"Close")
        new.Bind(wx.EVT_BUTTON,self.on_new);add.Bind(wx.EVT_BUTTON,self.on_add);edit.Bind(wx.EVT_BUTTON,self.on_edit);close.Bind(wx.EVT_BUTTON,lambda e:self.EndModal(wx.ID_CLOSE))
        buttons=wx.BoxSizer(wx.HORIZONTAL);buttons.Add(new);buttons.Add(add,0,wx.LEFT,8);buttons.Add(edit,0,wx.LEFT,8);buttons.AddStretchSpacer();buttons.Add(close)
...
def show_budgets(parent,connection,session,authorization):
    ...
    d=BudgetDialog(parent,BudgetService(connection,session.user_id))
    try:d.ShowModal()
    finally:d.Destroy()
