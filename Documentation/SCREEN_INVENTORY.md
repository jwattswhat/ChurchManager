...
|---|---|---|---|
| `frmMain` | ChurchManager + JSForm | Active; consistent accessible-button dashboard visually accepted August 30, 2026 | Not CM-only; JSForm renders the ChurchManager menu definition while ChurchManager opens the maintained PDF guide. |
...
| Group Meetings and Attendance | ChurchManager only | Active; Group-detail flow visually accepted; direct main-menu flow needs verification | ChurchManager-only because it combines Group privacy and lifecycle policy with effective membership rosters while remaining separate from worship attendance. |
| Projects and Scheduling | ChurchManager only | Active; visually accepted August 26, 2026 | Coordinates project lifecycle, ordered and dependent steps, Person/Group/User ownership, existing-document links, due work, and Calendar Integration eligibility. |
